		...
		if ($data_rawat != "") {
			...
			foreach ($data_rawat as $key) {
				echo '<tr>
                            ...
                            <td class="text-right">
                            	<a href="#" class="btn btn-info btn-xs" rel="tooltip" title="Detail Pasien" data-placement="bottom" onclick="detailOdontoPasien(' . $key->id_rawat . ');">Detail</a>
		...
		if ($data_rawat != "") {
			...
			foreach ($data_rawat as $key) {
				...
				if ($key->status == "DAFTAR") {
					...
					$status = '<button class="btn btn-rose btn-xs" style="margin: 0;">MENOLAK RAWAT</button>';
				} else {
