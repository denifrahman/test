...
class Rawat_history extends CI_Controller
{
	...
	function printPasien($id, $tipe)
	{
		...
		if ($data_pasien) {
			...
			$arrId4Sisi = array(
				'dental13', 'dental12', 'dental11', 'dental21', 'dental22', 'dental23',
	...
	function get_detail_odonto_by_id()
	{
		...
		if ($data['rawat']) {
			$odonto = json_decode(json_encode($data['rawat']['odontogram'][0]), true);
			$result = array_merge($odonto, array(
				"rc" => "0000"
			));
		} else {
			$result = array(
	...
	function get_detail_diagnosa()
	{
		...
		$header = '<thead>
                    ...
                    <tbody>';
		$no = 0;
		if ($data['rawat']['detail'] != "") {
			echo $header;
			foreach ($data['rawat']['detail'] as $key) {
				echo '<tr class="pilih-penyakit" data-nama="' . $key->diagnosa . '" data-id="' . $key->id . '">
                            <td>' . ($no + 1) . '</td>
                            <td>' . $key->diagnosa . '</td>
