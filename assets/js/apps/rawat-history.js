...
function editRawat(id) {
	...
	$.ajax({
		type: "POST",
		url: base_url + "rawat_history/get_detail_odonto_by_id",
		cache: false,
		data: JSON.stringify({
			"id": id
		}),
...
function getDetailDiagnosa(id) {
	...
	$.ajax({
		type: "POST",
		url: base_url + "rawat_history/get_detail_diagnosa",
		cache: false,
		data: JSON.stringify({
			"id": id
		}),
		success: function (json) {
			$("#tableDetail").show();
			$("#tableDetailDiagnosa").html(json);
			$('#tableDetailDiagnosa').DataTable().destroy();
			$('#tableDetailDiagnosa').DataTable({
				"pagingType": "full_numbers",
				"lengthMenu": [
