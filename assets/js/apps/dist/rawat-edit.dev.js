...
var by = function by(name, minor) {
  return function (o, p) {
    ...
    if (_typeof(o) === 'object' && _typeof(p) === 'object' && o && p) {
      a = o[name];
      b = p[name];
...
var odontogram = function odontogram() {
  var data = JSON.parse(odontogramData); // data = data.sort(by('id'));

  console.log(data.sort(by('id')));
  allDental = data; //hasil 
  ...
  var arrId4Sisi = ['dental13', 'dental12', 'dental11', 'dental21', 'dental22', 'dental23', 'dental53', 'dental52', 'dental51', 'dental61', 'dental62', 'dental63', 'dental83', 'dental82', 'dental81', 'dental71', 'dental72', 'dental73', 'dental43', 'dental42', 'dental41', 'dental31', 'dental32', 'dental33'];

  for (var i = 0; i < data.length; i++) {
    if (arrId4Sisi.indexOf(data[i].id) > -1) {
      var svg = '<g id="' + data[i].id + '" transform="translate(' + data[i].transform1 + ',' + data[i].transform2 + ')">' + '<polygon points="0,0   20,0    15,10    0,10" fill="' + data[i].data.T + '" stroke="navy" stroke-width="0.5" id="T" opacity="1" onclick="clickSvg(this);"></polygon>' + '<polygon points="5,10  15,10   20,20   0,20" fill="' + data[i].data.B + '" stroke="navy" stroke-width="0.5" id="B" opacity="1" onclick="clickSvg(this);"></polygon>' + '<polygon points="20,0  20,0    20,20   15,10" fill="' + data[i].data.R + '" stroke="navy" stroke-width="0.5" id="R" opacity="1" onclick="clickSvg(this);"></polygon>' + '<polygon points="0,0   5,10     5,10    0,20" fill="' + data[i].data.L + '" stroke="navy" stroke-width="0.5" id="L" opacity="1" onclick="clickSvg(this);"></polygon>' + '<text x="6" y="30" stroke="navy" fill="navy" stroke-width="0.1" style="font-size: 6pt;font-weight:normal">' + data[i].title + '</text>' + '</g>';
    } else {
      var svg = '<g id="' + data[i].id + '" transform="translate(' + data[i].transform1 + ',' + data[i].transform2 + ')">' + '<polygon points="5,5   15,5    15,15   5,15" fill="' + data[i].data.C + '" stroke="navy" stroke-width="0.5" id="C" opacity="1" onclick="clickSvg(this);"></polygon>' + '<polygon points="0,0   20,0    15,5    5,5" fill="' + data[i].data.T + '" stroke="navy" stroke-width="0.5" id="T" opacity="1" onclick="clickSvg(this);"></polygon>' + '<polygon points="5,15  15,15   20,20   0,20" fill="' + data[i].data.B + '" stroke="navy" stroke-width="0.5" id="B" opacity="1" onclick="clickSvg(this);"></polygon>' + '<polygon points="15,5  20,0    20,20   15,15" fill="' + data[i].data.R + '" stroke="navy" stroke-width="0.5" id="R" opacity="1" onclick="clickSvg(this);"></polygon>' + '<polygon points="0,0   5,5     5,15    0,20" fill="' + data[i].data.L + '" stroke="navy" stroke-width="0.5" id="L" opacity="1" onclick="clickSvg(this);"></polygon>' + '<text x="6" y="30" stroke="navy" fill="navy" stroke-width="0.1" style="font-size: 6pt;font-weight:normal">' + data[i].title + '</text>' + '</g>';
    }

    var parser = new DOMParser();
    var doc = parser.parseFromString(svg, "image/svg+xml"); // var baru = document.importNode(svg,true);

    document.getElementById("odontograma").appendChild(parseSVG(svg)); // console.log("svg: " , doc);
  ...
  $.ajax({
    ...
    success: function success(json) {
      ...
      for (var i = 0; i < obj.length; i++) {
        ...
        $("#id_bidang_ilmu").append("<option value='" + obj[i].id + "' " + selected + " data-nama='" + obj[i].nama + "'>" + obj[i].nama + "</option>"); // console.log("<option value='"+obj[i].id+"' "+selected+">"+obj[i].nama+"</option>");
      }
...
var tambahDetailPenyakit = function tambahDetailPenyakit() {
...
function saveHeaderRawat() {
  ...
  if (!$valid) {
    ...
    if (!$validDate) {
      ...
      $("body").addClass("loading"); // $("#tgl_rawat").val(parseDate($("#tgl_rawat").val()));
      ...
      $.ajax({
        ...
        success: function success(json) {
          ...
          if (obj.rc === "0000") {
            ...
            window.setTimeout(function () {// Move to a new location or you can do something else
              // window.location.href = base_url + 'rawat_history';
...
function saveDetailRawat() {
  ...
  if (!$valid) {
    ...
    var url = "";
    var src_foto = $("#foto_base64").val(); // $("#tgl_rawat").val(parseDate($("#tgl_rawat").val()));
    // console.log("xx: ", $("#fotos")[0].files[0]);
    ...
    $.ajax({
      ...
      success: function success(json) {
        ...
        if (obj.rc === "0000") {
          ...
          getUpdateTotalHargaRawat(obj.id_rawat); // $("#div_detail_penyakit").show();
  ...
  bootbox.confirm("Apakah anda yakin untuk menghapus penyakit [' " + nama + " '] dari daftar detail rawat?", function (result) {
    if (result) {
      $.ajax({
        ...
        error: function error(e) {// console.log("delete_pasien: ", e.responseText);
        }
      });
    }
  });
    ...
    $.ajax({
      ...
      success: function success(json) {
        ...
        if (obj.rc === "0000") {
          ...
          $("#harga").val(obj.harga);
          $("#harga_detail").val(obj.harga); // $("#fotos")[0].val(obj.foto);
...
function getDetailRawat(id) {
  ...
  $.ajax({
    ...
    success: function success(json) {
      ...
      $("#datatables-detail-rawat").html(json); // $('#datatables').DataTable().destroy();
